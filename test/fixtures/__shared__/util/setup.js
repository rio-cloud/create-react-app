...
module.exports = class TestSetup {
    ...
    async setup() {
        ...
        await fs.copy(path.resolve(__dirname, '..', 'template'), this.testDirectory);
        ...
        const packageJson = await fs.readJson(path.resolve(this.testDirectory, 'package.json'));
        ...
        await fs.writeJson(path.resolve(this.testDirectory, 'package.json'), packageJson);

        await execa(
            'yarnpkg',
            ['install', this.settings.pnp ? '--enable-pnp' : null, '--mutex', 'network'].filter(Boolean),
            {
        ...
        if (!shouldInstallScripts) {
            await fs.ensureSymlink(
                path.resolve(
                    path.resolve(__dirname, '../../../..', 'packages', 'react-scripts', 'bin', 'react-scripts.js')
                ),
