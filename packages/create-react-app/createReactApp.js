...
const program = new commander.Command(packageJson.name)
    ...
    .on('--help', () => {
        ...
        console.log(`      - a custom fork published on npm: ${chalk.green('cra-template-typescript')}`);
        console.log(
...
function createApp(name, verbose, version, template, useNpm, usePnp, useTypeScript) {
    ...
    if (useTypeScript) {
        console.log(
            chalk.yellow('The --typescript option has been deprecated and will be removed in a future release.')
        );
        console.log(chalk.yellow(`In future, please use ${chalk.cyan('--template typescript')}.`));
        console.log();
        if (!template) {
            template = 'typescript';
        }
    ...
    Promise.all([
        ...
        Promise.all([getPackageInfo(packageToInstall), getPackageInfo(templateToInstall)])
            ...
            .then(({ isOnline, packageInfo, templateInfo }) => {
                ...
                const templatesVersionMinimum = process.env.CRA_INTERNAL_TEST ? '3.2.0' : '3.3.0';
